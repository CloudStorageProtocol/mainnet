...
import com.uranus.library_wallet.base.mnemoinc.crypto.SecureRandomUtils;
...
//        System.out.println(Arrays.toString(mnemonic.toArray()));
    ...
    private static ETHWallet generateWallet(String walletName, String pwd, ECKeyPair ecKeyPair) {
        ...
        Lmsg.i("ETHWalletUtils", "publicKey = " + s);
        String wallet_dir = AppFilePath.Wallet_DIR;
        Lmsg.i("ETHWalletUtils", "wallet_dir = " + wallet_dir);
...
//            WalletFile walletFile = new Gson().fromJson(keystore, WalletFile.class);
            ...
            e.printStackTrace();
            Lmsg.e("ETHWalletUtils", e.toString());
...
//            ToastUtils.showToast(R.string.load_wallet_by_official_wallet_keystore_input_tip);
     ...
     * @return
     */
    public static String getPrivateKey(String pwd,String keystorePath) {

        Credentials credentials;
        ECKeyPair keypair;
        String privateKey = null;
        try {
            credentials = WalletUtils.loadCredentials(pwd,keystorePath);
            keypair = credentials.getEcKeyPair();
            privateKey = Numeric.toHexStringNoPrefixZeroPadded(keypair.getPrivateKey(), Keys.PRIVATE_KEY_LENGTH_IN_HEX);
        } catch (CipherException e) {
            e.printStackTrace();
     ...
     * @return
     */
    public static String getKeystore(String keystorePath) {
        String keystore = null;
        WalletFile walletFile;
        try {
            walletFile = objectMapper.readValue(new File(keystorePath), WalletFile.class);
            keystore = objectMapper.writeValueAsString(walletFile);
        } catch (IOException e) {
            e.printStackTrace();
        }
