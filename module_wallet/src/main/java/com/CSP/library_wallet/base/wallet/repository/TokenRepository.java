            ...
            for (int i = 0; i < tokens.size(); i++) {
                ...
                try {
                    if (tokenInfo.address.isEmpty()) {
                        ...
                        balance = getBalance(walletAddress, tokenInfo);
                    }
                ...
                Lmsg.d("balance:" + balance);

                if (balance == null || balance.compareTo(BigDecimal.ZERO) == 0) {
                    result[i] = new Token(tokenInfo, "0");
                } else {
                    ...
                    if (tokenInfo.decimals > 4) {
                        result[i] = new Token(tokenInfo, ethBalance.setScale(4, RoundingMode.CEILING).toPlainString());
                    } else {
                        result[i] = new Token(tokenInfo, ethBalance.setScale(tokenInfo.decimals, RoundingMode.CEILING).toPlainString());
                    }
...
//            e.onNext(tokens);
    ...
    public Completable addToken(String walletAddress, String address, String symbol, int decimals) {
        ...
        Lmsg.d("addToken:" + walletAddress + ", address: " + address + ", source:" + tokenLocalSource + ", ethereumNetworkRepository:" + ethereumNetworkRepository );
