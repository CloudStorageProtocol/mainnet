...
//    public static String URL = "http://192.168.1.138";
     ...
     * @param
     */
    public static void config(String url, String token) {
        URL = url;
        tokenAddres = token;
    }
     ...
     * @param keystore
     */
    public static String getPrivateKey(String password, String keystore){
        ECKeyPair ecKeyPair = null;
        String privateKey = null;
        try {
            ecKeyPair = WalletUtils.loadKeystore(password, keystore);
            privateKey =EthUtils.getPrivateKey(ecKeyPair);
        } catch (IOException e) {
            e.printStackTrace();
        } catch (CipherException e) {
            e.printStackTrace();
